...
uint8 UART_BCP_initVar = 0u;
        ...
        UART_BCP_SCB_MODE_EZI2C_CONST_CFG))
    /** This global variable stores TX interrupt sources after 
    * UART_BCP_Stop() is called. Only these TX interrupt sources 
    * will be restored on a subsequent UART_BCP_Enable() call.
    */
    uint16 UART_BCP_IntrTxMask = 0u;
...
****************************************************************************//**
*
*  Initializes the UART_BCP component to operate in one of the selected
*  configurations: I2C, SPI, UART or EZI2C.
*  When the configuration is set to "Unconfigured SCB", this function does
*  not do any initialization. Use mode-specific initialization APIs instead:
*  UART_BCP_I2CInit, UART_BCP_SpiInit, 
*  UART_BCP_UartInit or UART_BCP_EzI2CInit.
*
*******************************************************************************/
...
****************************************************************************//**
*
*  Enables UART_BCP component operation: activates the hardware and 
*  internal interrupt. It also restores TX interrupt sources disabled after the 
*  UART_BCP_Stop() function was called (note that level-triggered TX 
*  interrupt sources remain disabled to not cause code lock-up).
*  For I2C and EZI2C modes the interrupt is internal and mandatory for 
*  operation. For SPI and UART modes the interrupt can be configured as none, 
*  internal or external.
*  The UART_BCP configuration should be not changed when the component
*  is enabled. Any configuration changes should be made after disabling the 
*  component.
*  When configuration is set to “Unconfigured UART_BCP”, the component 
...
****************************************************************************//**
*
*  Invokes UART_BCP_Init() and UART_BCP_Enable().
...
****************************************************************************//**
*
*  Disables the UART_BCP component: disable the hardware and internal 
*  interrupt. It also disables all TX interrupt sources so as not to cause an 
*  unexpected interrupt trigger because after the component is enabled, the 
*  TX FIFO is empty.
*  Refer to the function UART_BCP_Enable() for the interrupt 
...
static void UART_BCP_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void UART_BCP_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            UART_BCP_RX_SCL_MOSI_HSIOM_SEL_GPIO,
            UART_BCP_TX_SDA_MISO_HSIOM_SEL_GPIO,
            0u,
            0u,
            0u,
            0u,
            ...
            (UART_BCP_SCB_MODE_EZI2C == mode))
        {
            ...
            hsiomSel[UART_BCP_RX_SCL_MOSI_PIN_INDEX] = UART_BCP_RX_SCL_MOSI_HSIOM_SEL_I2C;
            pinsDm  [UART_BCP_RX_SCL_MOSI_PIN_INDEX] = UART_BCP_PIN_DM_OD_LO;
        #elif (UART_BCP_RX_WAKE_SCL_MOSI_PIN)
            hsiomSel[UART_BCP_RX_WAKE_SCL_MOSI_PIN_INDEX] = UART_BCP_RX_WAKE_SCL_MOSI_HSIOM_SEL_I2C;
            pinsDm  [UART_BCP_RX_WAKE_SCL_MOSI_PIN_INDEX] = UART_BCP_PIN_DM_OD_LO;
        #else
        #endif /* (UART_BCP_RX_SCL_MOSI_PIN) */
        
        #if (UART_BCP_TX_SDA_MISO_PIN)
            hsiomSel[UART_BCP_TX_SDA_MISO_PIN_INDEX] = UART_BCP_TX_SDA_MISO_HSIOM_SEL_I2C;
        ...
        {
        #if (UART_BCP_RX_SCL_MOSI_PIN)
            hsiomSel[UART_BCP_RX_SCL_MOSI_PIN_INDEX] = UART_BCP_RX_SCL_MOSI_HSIOM_SEL_SPI;
        #elif (UART_BCP_RX_WAKE_SCL_MOSI_PIN)
            hsiomSel[UART_BCP_RX_WAKE_SCL_MOSI_PIN_INDEX] = UART_BCP_RX_WAKE_SCL_MOSI_HSIOM_SEL_SPI;
        #else
        #endif /* (UART_BCP_RX_SCL_MOSI_PIN) */
        
        #if (UART_BCP_TX_SDA_MISO_PIN)
            hsiomSel[UART_BCP_TX_SDA_MISO_PIN_INDEX] = UART_BCP_TX_SDA_MISO_HSIOM_SEL_SPI;
        #endif /* (UART_BCP_TX_SDA_MISO_PIN) */
        
        #if (UART_BCP_SCLK_PIN)
            hsiomSel[UART_BCP_SCLK_PIN_INDEX] = UART_BCP_SCLK_HSIOM_SEL_SPI;
            ...
            if (UART_BCP_SPI_SLAVE == subMode)
            {
                ...
                hsiomSel[UART_BCP_SS0_PIN_INDEX] = UART_BCP_SS0_HSIOM_SEL_SPI;
        ...
        else /* UART */
        {
            if (UART_BCP_UART_MODE_SMARTCARD == subMode)
            {
                /* SmartCard */
            #if (UART_BCP_TX_SDA_MISO_PIN)
                hsiomSel[UART_BCP_TX_SDA_MISO_PIN_INDEX] = UART_BCP_TX_SDA_MISO_HSIOM_SEL_UART;
                pinsDm  [UART_BCP_TX_SDA_MISO_PIN_INDEX] = UART_BCP_PIN_DM_OD_LO;
            #endif /* (UART_BCP_TX_SDA_MISO_PIN) */
            }
            else /* Standard or IrDA */
            {
                ...
                {
                #if (UART_BCP_RX_SCL_MOSI_PIN)
                    hsiomSel[UART_BCP_RX_SCL_MOSI_PIN_INDEX] = UART_BCP_RX_SCL_MOSI_HSIOM_SEL_UART;
                    pinsDm  [UART_BCP_RX_SCL_MOSI_PIN_INDEX] = UART_BCP_PIN_DM_DIG_HIZ;
                #elif (UART_BCP_RX_WAKE_SCL_MOSI_PIN)
                    hsiomSel[UART_BCP_RX_WAKE_SCL_MOSI_PIN_INDEX] = UART_BCP_RX_WAKE_SCL_MOSI_HSIOM_SEL_UART;
                    pinsDm  [UART_BCP_RX_WAKE_SCL_MOSI_PIN_INDEX] = UART_BCP_PIN_DM_DIG_HIZ;
                #else
                #endif /* (UART_BCP_RX_SCL_MOSI_PIN) */
                }
                ...
                if (UART_BCP_UART_MODE_STD == subMode)
                {
                    ...
                    #if (UART_BCP_SCLK_PIN)
                        hsiomSel[UART_BCP_SCLK_PIN_INDEX] = UART_BCP_SCLK_HSIOM_SEL_UART;
                        pinsDm  [UART_BCP_SCLK_PIN_INDEX] = UART_BCP_PIN_DM_DIG_HIZ;
                    #endif /* (UART_BCP_SCLK_PIN) */
                    }
                    ...
                    #if (UART_BCP_SS0_PIN)
                        hsiomSel[UART_BCP_SS0_PIN_INDEX] = UART_BCP_SS0_HSIOM_SEL_UART;
                        pinsDm  [UART_BCP_SS0_PIN_INDEX] = UART_BCP_PIN_DM_STRONG;
